...
The backend's start command already ran the seed script for you on first boot, but its one-time output (a randomly generated Super Admin username/password) only ever appears in one place:
...
- **Database (~every 30 days):** the free Postgres instance expires and enters a 14-day grace period before deletion. Before it's deleted: either upgrade it to a paid instance (a few dollars/month, keeps everything as-is), or create a fresh free database, point the backend's `DATABASE_URL` at it, and redeploy - the start command re-runs migrations and seeding automatically (you will lose existing data unless you `pg_dump` it out first from the old instance and restore into the new one while both still exist).
