...
| Email | `MAIL_PROVIDER=console` (logs instead of sending) | No real email is ever sent. To keep this from being a dead end, the app compensates automatically: the create-organization response echoes back the new admin's temp password whenever no real mail provider is configured (regardless of `NODE_ENV`), and the Organizations screen has an **Activate** button that skips the emailed activation token entirely - see Step 4. |
...
- Nothing else - no CLI to install, no Docker to install, no cloud credentials to configure. Render builds the backend and frontend directly from your GitHub repo using its own Node runtime (`backend/Dockerfile.compose` and `frontend/Dockerfile.compose` in this repo are used only for the docker-compose local-dev setup and the AWS/Azure guides - Render itself never touches them; they're deliberately *not* named `Dockerfile`, since some platforms auto-detect and silently build from a file with that exact name even when told to use a different builder - see deployment/09-Deployment-Railway.md for a concrete case where this happened).
...
1. Dashboard -> backend service -> **Logs** -> scroll/search near the first deploy for a line like `Super Admin created. TEMP PASSWORD: ...`. (Alternatively, the seed script also creates a ready-to-use **Demo Facilities Inc** organization with three working, already-activated logins - `orgadmin` / `storekeeper` / `hruser`, all password `Demo@12345` unless you set `SEED_DEMO_USER_PASSWORD` - if you just want something to click around in immediately, skip straight to logging in with one of those instead of the steps below.)
2. Log in to the frontend with the Super Admin username/password, change the password when prompted (forced on first login), then use the **Organizations** screen to create your first tenant organization.
3. Because `MAIL_PROVIDER=console` on this deployment, the create-organization response includes the new admin's username and temp password directly on screen - no logs required. Click the **Activate** button next to the pending organization (this calls a Super-Admin-only endpoint that skips the emailed activation token entirely, since you're the same person who just created the org), then log out and log in as that admin using the credentials shown in step 2 to finish setup and start creating the users who will actually test the app.
