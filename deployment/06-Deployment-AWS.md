...
- Docker installed locally, and the backend/frontend Docker images buildable (see repo `Dockerfile.compose` in each of `backend/` and `frontend/` once code phase begins - named `.compose` rather than the default `Dockerfile` so Render's and Railway's native-runtime builders don't auto-detect and silently use them instead of those platforms' own build/start commands).
...
1. Build images locally or in CI:
   ```
   docker build -f backend/Dockerfile.compose -t spqr-backend ./backend
   docker build -f frontend/Dockerfile.compose -t spqr-frontend ./frontend
   ```
