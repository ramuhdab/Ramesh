...
`RAILPACK_BUILD_APT_PACKAGES`/`RAILPACK_DEPLOY_APT_PACKAGES` matter because Railway's native (non-Docker) builder doesn't include the `openssl` system package by default, which Prisma's engine needs to detect the runtime's libssl/OpenSSL version. Without it, the deploy logs show `Prisma failed to detect the libssl/openssl version... Defaulting to openssl-1.1.x` - a guess that doesn't always match reality and can cause real query failures, not just a cosmetic warning.
