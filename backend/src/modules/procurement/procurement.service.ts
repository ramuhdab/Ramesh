...
export async function createProcurementRequest(
  ...
  await approvalService.startApprovalChain(organizationId, "procurement_request", request.id, "procurement");

  eventBus.publish({ type: "procurement.requested", organizationId, actorUserId, payload: { id: request.id } });
...
export async function createIndent(organizationId: string, input: { departmentId?: string; items: unknown }, raisedByUserId: string) {
  ...
  await approvalService.startApprovalChain(organizationId, "indent", indent.id, "indent");

  eventBus.publish({ type: "procurement.indent_raised", organizationId, actorUserId: raisedByUserId, payload: { id: indent.id } });
...
export async function listProcurementRequests(organizationId: string) {
  return prisma.procurementRequest.findMany({ where: { organizationId }, orderBy: { createdAt: "desc" } });
}

export async function listIndents(organizationId: string) {
  return prisma.indent.findMany({ where: { organizationId }, orderBy: { createdAt: "desc" } });
}
