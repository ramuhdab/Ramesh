...
export async function changePassword(userId: string, currentPassword: string, newPassword: string, isSuperAdmin = false) {
  // Sparquer Super Administrators live in a separate table (see login()
  // above) - without this branch, this always queried the `User` table,
  // which meant a Super Admin could never complete the forced first-login
  // password change (mustChangePassword: true from prisma/seed.ts) at all.
  if (isSuperAdmin) {
    const admin = await prisma.superAdmin.findUniqueOrThrow({ where: { id: userId } });
    ...
    // No password-history/reuse check here: PasswordHistory.userId is a
    // real foreign key to User specifically (FR-5's "last 5 passwords"
    // rule targets tenant users), and there's a single Super Admin account
    // rather than many, so reuse-tracking doesn't apply the same way.
    const newHash = await hashPassword(newPassword);
    await prisma.superAdmin.update({ where: { id: userId }, data: { passwordHash: newHash, mustChangePassword: false } });
    return;
  }
